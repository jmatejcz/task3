  ...
  if (jwt_token == null) {
    ...
    JWT.verify(
      ...
      secret_key,
      {
        algorithms: ["HS256"],
        complete: true,
        audience: "https://127.0.0.1/jwt/none",
      },
      (err, decoded_token) => {
        if (err) {
...
app.post("/jwt/weak-secret", (req, res) => {
  ...
  if (jwt_token === null) {
    ...
    JWT.verify(
      ...
      secret_key,
      {
        algorithm: "HS256",
        complete: true,
        audience: "https://127.0.0.1/jwt/weak-secret",
      },
      (err, decoded_token) => {
        if (err) {
          res.status(400).json(err);
...
app.post("/jwt/key-confusion", (req, res) => {
  //key-confusion endpoint
  const { jwt_token } = req.body;
  ...
  if (jwt_token === null) {
    ...
    const publicKey = fs.readFileSync(
      `${__dirname}/certificate/public_key_kca.crt`
    );
    JWT.verify(
      jwt_token,
      publicKey,
      {
        ...
        complete: true,
        audience: "https://127.0.0.1/jwt/key-confusion",
      },
      (err, decoded_token) => {
        if (err) {
          res.status(400).json(err);
...
app.post("/jwt/key-injection", (req, res) => {
  ...
  if (jwt_token === null) {
    ...
    JWT.verify(
      ...
      thepublicKey,
      {
        algorithm: "RS256",
        complete: true,
        audience: "https://127.0.0.1/jwt/key-injection",
      },
      (err, decoded_token) => {
        if (err) {
          res.status(400).json(err);
        ...
        if (response.statusCode == 200) {
          ...
          JWT.verify(
            ...
            thepublicKey,
            {
              algorithm: "RS256",
              complete: true,
              audience: "https://127.0.0.1/jwt/jku",
            },
            (err, decoded_token) => {
              if (err) {
                res.status(400).json(err);
          ...
          res
            .status(500)
            .json(
              `An error has ocurred reaching ${jwt_b64_dec.header.jku}, status: ${response.statusCode}`
            );
        }
      }
    });
  ...
  if (jwt_token === null) {
    ...
    request(jwt_b64_dec.header.x5u, (error, response, body) => {
      if (error) {
        ...
        if (response.statusCode == 200) {
          fs.writeFile(
            `${__dirname}/certificate/temp_x5u.cert`,
            body,
            (err) => {
              if (err) {
                ...
                exec(
                  ` openssl x509 -in ${__dirname}/certificate/temp_x5u.cert -noout -pubkey`,
                  (error, x509cert, stderr) => {
                    if (error) {
                    ...
                    JWT.verify(
                      ...
                      x509cert,
                      {
                        algorithm: "RS256",
                        complete: true,
                        audience: "https://127.0.0.1/jwt/x5u",
                      },
                      (err, decoded_token) => {
                        if (err) {
                          res.status(400).json(err);
                          ...
                          const success = {
                            message:
                              "Congrats!! You've solved the JWT challenge!!",
                            jwt_token: decoded_token,
                          };
                          res.status(200).json(success);
                        }
...
app.post("/jwt/kid00", (req, res) => {
  //kid endpoint command execution
  const { jwt_token } = req.body;
  ...
  if (jwt_token === null) {
    ...
    if (jwt_b64_dec.header.kid) {
      exec(
        `cat ${__dirname}/secrets/${jwt_b64_dec.header.kid}`,
        (error, secret_key, stderr) => {
          if (error) {
          ...
          JWT.verify(
            ...
            secret_key,
            {
              algorithm: "HS256",
              complete: true,
              audience: "https://127.0.0.1/jwt/kid00",
            },
            (err, decoded_token) => {
              if (err) {
                res.status(400).json(err);
...
app.get("/webkeys/certificate_x509.crt", (req, res) => {
  //makes the certificate available, this is for xsu attack
...
app.get("/webkeys/jwks.json", (req, res) => {
  //makes the json web key available, this is for jku attack
...
app.post("/jwt", (req, res) => {
  ...
  if (attack === "none") {
    JWT.sign(
      payload,
      "885ae2060fbedcfb491c5e8aafc92cab5a8057b3d4c39655acce9d4f09280a20",
      { algorithm: "HS256", audience: "https://127.0.0.1/jwt/none" },
      (err, token) => {
        const res_body = {
          jwt: token,
  ...
  if (attack === "weak-secret") {
    ...
    JWT.sign(
      payload,
      secret_key,
      { algorithm: "HS256", audience: "https://127.0.0.1/jwt/weak-secret" },
      (err, token) => {
        const res_body = {
          jwt: token,
  ...
  if (attack === "key-confusion") {
    const privateKey = fs.readFileSync(
      `${__dirname}/certificate/private_key_kca.key`
    );
    JWT.sign(
      payload,
      privateKey,
      { algorithm: "RS256", audience: "https://127.0.0.1/jwt/key-confusion" },
      (err, token) => {
        const res_body = {
          jwt: token,
  ...
  if (attack === "key-injection") {
    ...
    const privateKey_kia = fs.readFileSync(
      `${__dirname}/certificate/private_key_kia.key`
    );
    JWT.sign(
      payload,
      privateKey_kia,
  ...
  if (attack === "jku") {
    ...
    JWT.sign(
      ...
      privateKey_kia,
      {
        algorithm: "RS256",
        audience: "https://127.0.0.1/jwt/jku",
        header: { jku: "https://127.0.0.1/webkeys/jwks.json" },
      },
      (err, token) => {
        const res_body = {
          jwt: token,
  ...
  if (attack === "x5u") {
    const privateKey_x509 = fs.readFileSync(
      `${__dirname}/certificate/private_key_x509.key`
    );
    JWT.sign(
      payload,
      privateKey_x509,
      {
        algorithm: "RS256",
        audience: "https://127.0.0.1/jwt/x5u",
        header: { x5u: "https://127.0.0.1/webkeys/certificate_x509.crt" },
      },
      (err, token) => {
        const res_body = {
          jwt: token,
  ...
  if (attack === "kid00") {
    exec(
      `cat ${__dirname}/secrets/keyfile.txt`,
      (error, secret_key, stderr) => {
        if (error) {
          res.status(200).send(error);
        }
...
https
  .createServer(
    {
      //Use a certificate to provide TLS
      key: fs.readFileSync(
        `${__dirname}/certificate/private_key_kca.key`,
        (err, data) => {
          console.log(err);
      ...
      cert: fs.readFileSync(
        `${__dirname}/certificate/certificate_kca.crt`,
        (err, data) => {
          console.log(err);
        }
      ),
